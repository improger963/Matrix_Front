...
const Wallet: React.FC = () => {
    ...
    const [foundRecipient, setFoundRecipient] = useState<Pick<User, 'id' | 'name' | 'avatarUrl'> | null>(null);
    ...
    const handleFindRecipient = async (e: React.FormEvent) => {
        ...
        if (recipient) {
            ...
            setSearchError(`Пользователь с ID "${recipientId}" не найден.`);
            setFoundRecipient(null);
        }
    };
    ...
    const getTransactionRow = (transaction: Transaction) => {
        ...
        if (transaction.type === 'transfer') {
            ...
            typeInfo = typeMap[transaction.type];
        }
    ...
    const renderDepositForm = () => (
        <form onSubmit={handleFormSubmit} className="space-y-4 max-w-md mx-auto">
            <h3 className="text-lg font-semibold text-white text-center">Сумма пополнения ($)</h3>
            <div>
                <input
                    type="number"
                    value={amount}
            ...
            <h3 className="text-lg font-semibold text-white text-center">Способ пополнения</h3>
    ...
    const renderCryptoDetails = () => (
         <div className="max-w-md mx-auto text-center space-y-4">
            <button onClick={() => setStep('form')} className="flex items-center gap-2 text-sm text-brand-accent hover:text-white mb-4">
             ...
             <h3 className="text-lg font-semibold text-white">Пополнение на ${transactionDetails?.amount} USDT</h3>
    ...
    const renderConfirmation = () => {
        ...
        return (
             <div className="max-w-md mx-auto text-center space-y-4">
                <h3 className="text-xl font-bold text-white">Подтвердите вывод</h3>
    ...
    const renderTransferForm = () => (
        <form onSubmit={handleFindRecipient} className="space-y-4 max-w-md mx-auto">
            <h3 className="text-lg font-semibold text-white text-center">Внутренний перевод</h3>
            
            <div>
                 <label htmlFor="recipientId" className="block text-sm font-medium text-gray-300 mb-1">ID получателя</label>
                <input
                    id="recipientId"
                    type="text"
                    value={recipientId}
             ...
             <div>
                <label htmlFor="transferAmount" className="block text-sm font-medium text-gray-300 mb-1">Сумма перевода ($)</label>
                <input
                    id="transferAmount"
                    type="number"
                    value={amount}
        ...
        <div className="max-w-md mx-auto text-center space-y-4">
            ...
            <p className="text-gray-400">
                 {transactionDetails?.type === 'transfer' ?
                    ...
                    :
                    `Ваша заявка на ${transactionDetails?.type === 'deposit' ? 'пополнение' : 'вывод'} на сумму $${transactionDetails?.amount.toFixed(2)} была успешно создана.`
                }
            </p>
            <Button onClick={resetFlow} className="w-full !py-3">Вернуться в кошелек</Button>
        </div>
    );

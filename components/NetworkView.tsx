...
import { MOCK_NETWORK_MEMBERS, MOCK_ACHIEVEMENTS } from '../constants.ts';
import { Users, BarChart3, Zap, Award, GraduationCap, Flame, Rocket, Shield } from 'lucide-react';
import type { Partner } from '../types.ts';

import NetworkAnalyticsView from './NetworkAnalyticsView.tsx';
import NetworkListView from './NetworkListView.tsx';
import NetworkAchievementsView from './NetworkAchievementsView.tsx';

type Tab = 'analytics' | 'partners' | 'achievements';

interface NetworkViewProps {
    initialTab?: string | null;
}

const NetworkView: React.FC<NetworkViewProps> = ({ initialTab }) => {
    const isValidTab = (tab: string | null | undefined): tab is Tab => {
        return tab === 'analytics' || tab === 'partners' || tab === 'achievements';
    };
    
    const [activeTab, setActiveTab] = useState<Tab>(isValidTab(initialTab) ? initialTab : 'analytics');

    const teamAnalytics = useMemo(() => {
        const team: Partner[] = MOCK_NETWORK_MEMBERS;
        ...
        const topReferrers = [...team].sort((a, b) => b.partners - a.partners).slice(0, 3);
        ...
        if (topReferrers.length > 0 && topReferrers[0].partners > 3) insights.push({ icon: Award, text: `${topReferrers[0].name} — ваш лучший партнер. Отметьте его успехи!`, type: 'info' });
        if (averageLevel < 2 && insights.length < 2) insights.push({ icon: GraduationCap, text: `Средний уровень Бизнес-сети невысок. Сконцентрируйтесь на обучении.`, type: 'info' });
        if (insights.length === 0) insights.push({ icon: Award, text: `Отличные показатели! Бизнес-сеть работает эффективно.`, type: 'success' });
        ...
        <div className="space-y-6">
            <div className="border-b border-dark-700">
                <nav className="-mb-px flex gap-2 sm:gap-4 overflow-x-auto">
                    ...
                    <button onClick={() => setActiveTab('partners')} className={`flex items-center gap-2 px-1 sm:px-4 py-3 font-semibold border-b-2 whitespace-nowrap ${activeTab === 'partners' ? 'text-brand-primary border-brand-primary' : 'text-gray-400 border-transparent hover:text-white hover:border-gray-500'}`}><Users className="h-5 w-5"/>Партнеры</button>
                    <button onClick={() => setActiveTab('achievements')} className={`flex items-center gap-2 px-1 sm:px-4 py-3 font-semibold border-b-2 whitespace-nowrap ${activeTab === 'achievements' ? 'text-brand-primary border-brand-primary' : 'text-gray-400 border-transparent hover:text-white hover:border-gray-500'}`}><Shield className="h-5 w-5"/>Зал Славы</button>
                </nav>
            </div>
            
            <div className="animate-fade-in">
                {activeTab === 'analytics' && <NetworkAnalyticsView teamAnalytics={teamAnalytics} />}
                {activeTab === 'partners' && <NetworkListView />}
                {activeTab === 'achievements' && <NetworkAchievementsView achievementStats={achievementStats} isHallOfFame={true}/>}
            </div>
        </div>
    );
};

...
import { MOCK_GUILD_MEMBERS, MOCK_ACHIEVEMENTS } from '../constants.ts';
...
const GuildView: React.FC<GuildViewProps> = ({ initialTab }) => {
    ...
    const teamAnalytics = useMemo(() => {
        ...
        if (newLast7Days >= 5) insights.push({ icon: Rocket, text: `Отличный темп роста! ${newLast7Days} новых инвесторов за неделю.`, type: 'success' });
        ...
        if (averageLevel < 2 && insights.length < 2) insights.push({ icon: GraduationCap, text: `Средний уровень гильдии невысок. Сконцентрируйтесь на обучении.`, type: 'info' });
        if (insights.length === 0) insights.push({ icon: Award, text: `Отличные показатели! Гильдия работает эффективно.`, type: 'success' });
        ...
        <div className="space-y-6">
            <div className="border-b border-dark-700">
                <nav className="-mb-px flex gap-2 sm:gap-4 overflow-x-auto">
                    ...
                    <button onClick={() => setActiveTab('guild')} className={`flex items-center gap-2 px-1 sm:px-4 py-3 font-semibold border-b-2 whitespace-nowrap ${activeTab === 'guild' ? 'text-brand-primary border-brand-primary' : 'text-gray-400 border-transparent hover:text-white hover:border-gray-500'}`}><Users className="h-5 w-5"/>Члены Гильдии</button>

...
type Step = 'form' | 'crypto_details' | 'verifying' | 'confirm_withdrawal' | 'success' | 'transfer_confirm';
...
const TransactionAnalytics: React.FC = () => {
    const data = [
        { name: 'Прибыль от Проектов', value: 400, color: '#10b981' },
        { name: 'Прибыль от Бизнес-сети', value: 300, color: '#2dd4bf' },
        { name: 'Запуск Проектов', value: -200, color: '#f87171' },
        { name: 'Масштабирование', value: -100, color: '#f43f5e' },
    ];
                 ...
                 <div>
                    <div className="mb-2">
                     ...
                     <div>
                        ...
                        <p className="text-xl font-bold text-red-400">${Math.abs(expenses).toFixed(2)}</p>
                    </div>
                </div>
            </div>
        </Card>
...
const AssetManagementView: React.FC = () => {
    ...
    const MOCK_WALLET_ADDRESS = "TXYZ123abcdeFGHIJKLMNopqrstuvwxyz789";
     ...
     useEffect(() => {
        let timer: number;
        if (step === 'crypto_details' && countdown > 0) {
    ...
    const handleCopy = () => {
        ...
        setCopied(true);
        addToast('Адрес кошелька скопирован!', 'success');
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (activeTab === 'deposit') {
            ...
            if (paymentMethod === 'crypto') {
                ...
                addToast('Симуляция: Переход на страницу оплаты...', 'info');
                setStep('verifying');
                 setTimeout(() => {
                    setUser(prev => ({ ...prev, capital: prev.capital + numericAmount }));
                    setStep('success');
                }, 3000);
            }
    ...
    const handlePaid = () => {
        ...
        setTimeout(() => {
            if (transactionDetails) {
                 setUser(prev => ({...prev, capital: prev.capital + transactionDetails.amount}));
                 addToast(`Баланс успешно пополнен на $${transactionDetails.amount.toFixed(2)} CAP!`, 'success');
            }
            setStep('success');
        }, 3000);
    };
    ...
    const handleConfirmTransfer = () => {
        const numericAmount = Number(amount);
        setUser(prev => ({ ...prev, capital: prev.capital - numericAmount * 1.01 }));
        setTransactionDetails({ type: 'transfer', amount: numericAmount, recipient: foundRecipient! });
        addToast(`Перевод для ${foundRecipient?.name} успешно выполнен!`, 'success');
        setStep('success');
    };
    
    const handleConfirmWithdrawal = () => {
        const numericAmount = transactionDetails?.amount || 0;
        setUser(prev => ({ ...prev, capital: prev.capital - numericAmount }));
        addToast(`Заявка на продажу $${numericAmount.toFixed(2)} CAP создана!`, 'success');
        setStep('success');
    };
    ...
    const renderCryptoDetails = () => (
         <div className="max-w-md mx-auto text-center space-y-4">
            <button onClick={resetFlow} className="flex items-center gap-2 text-sm text-brand-accent hover:text-white mb-4">
                <ArrowLeft className="h-4 w-4" /> Назад
            </button>
             <h3 className="text-lg font-semibold text-white">Покупка ${transactionDetails?.amount} CAP</h3>
            <p className="text-sm text-gray-400">Отсканируйте QR-код или скопируйте адрес. Заявка действительна: <span className="font-mono text-white">{`${Math.floor(countdown / 60)}:${(countdown % 60).toString().padStart(2, '0')}`}</span></p>
            <div className="bg-white p-4 rounded-lg inline-block">
                <QRCode value={MOCK_WALLET_ADDRESS} size={160} />
            </div>
            <div className="p-3 bg-dark-900 border border-dark-700 rounded-lg">
             ...
             <Button onClick={handlePaid} className="w-full !py-3">Я оплатил</Button>
         </div>
    );
     ...
     const renderWithdrawalForm = () => (
        <form onSubmit={handleFormSubmit} className="space-y-4 max-w-md mx-auto">
            ...
            />
             <h3 className="text-lg font-semibold text-white">Адрес кошелька (USDT TRC-20)</h3>
             <input
                type="text"
                value={address}
                onChange={(e) => setAddress(e.target.value)}
                placeholder="Введите ваш USDT (TRC-20) адрес"
                className="w-full p-3 bg-dark-900 border border-dark-700 rounded-lg focus:ring-2 focus:ring-brand-primary focus:outline-none transition-all"
            />
            {error && <p className="text-red-500 text-center text-sm">{error}</p>}
    ...
    const renderVerifying = () => (
         <div className="max-w-md mx-auto text-center space-y-4">
            <LoaderCircle className="w-16 h-16 text-brand-primary animate-spin mx-auto" />
            <h3 className="text-xl font-bold text-white">Проверяем платеж...</h3>
            <p className="text-gray-400">Это может занять до 60 секунд. Пожалуйста, не закрывайте эту страницу.</p>
        </div>
    );
    ...
    const renderSuccess = () => (
        <div className="max-w-md mx-auto text-center space-y-4">
            ...
            <h3 className="text-xl font-bold text-white">Операция успешно выполнена!</h3>
            ...
            <Button onClick={resetFlow} className="w-full !py-3">Новая операция</Button>
        </div>
    );
        ...
        <div className="space-y-6">
            <Card>
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-center md:text-left">
                    <div>
                        <h2 className="text-2xl font-bold text-white">Управление Активами</h2>

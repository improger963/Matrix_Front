...
const EventIcon: React.FC<{ type: LiveFeedEvent['type'] }> = ({ type }) => {
    const iconMap = {
        ...
        matrix_close: <ShieldCheck className="h-5 w-5 text-purple-400" />,
    };
...
const EventMessage: React.FC<{ event: LiveFeedEvent }> = ({ event }) => {
    ...
    switch (type) {
        ...
        case 'matrix_close': return <>{userName} закрыл(а) матрицу и получил(а) <span className="font-semibold text-purple-400">${amount?.toFixed(2)}</span>.</>;
        default: return null;
    }
};

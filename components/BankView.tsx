...
import type { Transaction, Tycoon } from '../types.ts';
import { DollarSign, ArrowUpCircle, ArrowDownCircle, History, CreditCard, Bitcoin, Landmark, CheckCircle, ArrowLeft, Copy, Check, Send, UserSearch, LoaderCircle, Building } from 'lucide-react';
...
interface TransactionDetails {
    ...
    address?: string;
    method?: 'card' | 'crypto';
    recipient?: Pick<Tycoon, 'id' | 'name' | 'avatarUrl'>;
}
...
const BankView: React.FC = () => {
    ...
    const [foundRecipient, setFoundRecipient] = useState<Pick<Tycoon, 'id' | 'name' | 'avatarUrl'> | null>(null);
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (activeTab === 'deposit') {
            ...
            if (numericAmount > user.cityCredits) {
    ...
    const handleFindRecipient = async (e: React.FormEvent) => {
        ...
        if (recipient) {
            ...
            setSearchError(`Магнат с ID "${recipientId}" не найден.`);
            setFoundRecipient(null);
        }
    };
    ...
    const getTransactionRow = (transaction: Transaction) => {
        const typeMap = {
            deposit: { text: 'Покупка ГК', icon: <ArrowUpCircle className="h-5 w-5 text-green-500" /> },
            withdrawal: { text: 'Продажа ГК', icon: <ArrowDownCircle className="h-5 w-5 text-red-500" /> },
            earning: { text: 'Арендная плата', icon: <DollarSign className="h-5 w-5 text-yellow-500" /> },
            activation: { text: 'Покупка участка', icon: <Landmark className="h-5 w-5 text-blue-500" /> },
            upgrade: { text: 'Улучшение проекта', icon: <Building className="h-5 w-5 text-purple-400" /> },
        };
    ...
    const renderDepositForm = () => (
        <form onSubmit={handleFormSubmit} className="space-y-4 max-w-md mx-auto">
            <h3 className="text-lg font-semibold text-white text-center">Сумма покупки (ГК)</h3>
            <div>
                <input
                    type="number"
                    value={amount}
    ...
    const renderCryptoDetails = () => (
         <div className="max-w-md mx-auto text-center space-y-4">
            <button onClick={() => setStep('form')} className="flex items-center gap-2 text-sm text-brand-accent hover:text-white mb-4">
             ...
             <h3 className="text-lg font-semibold text-white">Покупка ©{transactionDetails?.amount} ГК</h3>
     ...
     const renderWithdrawalForm = () => (
        <form onSubmit={handleFormSubmit} className="space-y-4 max-w-md mx-auto">
            <h3 className="text-lg font-semibold text-white">Сумма продажи (ГК)</h3>
             <input
                type="number"
                value={amount}
                onChange={(e) => setAmount(e.target.value)}
            ...
            />
            {error && <p className="text-red-500 text-center text-sm">{error}</p>}
            <p className="text-xs text-gray-500 text-center">Комиссия за вывод: 2%. Минимальная сумма: ©100.</p>
            <Button type="submit" className="w-full !py-3">Продолжить</Button>
        </form>
    );
    ...
    const renderConfirmation = () => {
        ...
        return (
             <div className="max-w-md mx-auto text-center space-y-4">
                <h3 className="text-xl font-bold text-white">Подтвердите продажу ГК</h3>
                ...
                <div className="text-left bg-dark-900/50 p-4 rounded-lg space-y-3">
                    <div className="flex justify-between">
                        ...
                        <span className="font-semibold text-white">©{details.amount.toFixed(2)}</span>
                    </div>
                     <div className="flex justify-between">
                     ...
                     <div className="flex justify-between border-t border-dark-700 pt-3">
                        ...
                        <span className="font-bold text-green-400 text-lg">${total.toFixed(2)}</span>
                    </div>
                     <div className="flex justify-between">
    ...
    const renderTransferForm = () => (
        <form onSubmit={handleFindRecipient} className="space-y-4 max-w-md mx-auto">
            <h3 className="text-lg font-semibold text-white text-center">Внутренний перевод ГК</h3>
            
            <div>
                 <label htmlFor="recipientId" className="block text-sm font-medium text-gray-300 mb-1">ID Магната-получателя</label>
                <input
                    id="recipientId"
                    type="text"
                    value={recipientId}
             ...
             <div>
                <label htmlFor="transferAmount" className="block text-sm font-medium text-gray-300 mb-1">Сумма перевода (ГК)</label>
                <input
                    id="transferAmount"
                    type="number"
                    value={amount}
            ...
            <div className="max-w-md mx-auto space-y-4">
                ...
                <div className="text-left bg-dark-900/50 p-4 rounded-lg space-y-3">
                    <div className="flex justify-between">
                        ...
                        <span className="font-semibold text-white">©{numericAmount.toFixed(2)}</span>
                    </div>
                     <div className="flex justify-between">
                     ...
                     <div className="flex justify-between border-t border-dark-700 pt-3">
                        ...
                        <span className="font-bold text-red-400 text-lg">©{total.toFixed(2)}</span>
                    </div>
                </div>
        ...
        <div className="max-w-md mx-auto text-center space-y-4">
            ...
            <p className="text-gray-400">
                 {transactionDetails?.type === 'transfer' ?
                    `Ваш перевод на сумму ©${Number(amount).toFixed(2)} для пользователя ${transactionDetails.recipient?.name} успешно отправлен.`
                    :
                    `Ваша заявка на ${transactionDetails?.type === 'deposit' ? 'покупку' : 'продажу'} на сумму ©${transactionDetails?.amount.toFixed(2)} была успешно создана.`
                }
            </p>
            <Button onClick={resetFlow} className="w-full !py-3">Вернуться в Банк</Button>
        </div>
    );
    ...
    const tabs: {id: Tab, label: string, icon: React.ReactNode}[] = [
        { id: 'deposit', label: 'Купить ГК', icon: <ArrowUpCircle className="h-5 w-5"/> },
        { id: 'withdraw', label: 'Продать ГК', icon: <ArrowDownCircle className="h-5 w-5"/> },
        ...
        <div className="space-y-6">
            <Card>
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-center md:text-left">
                    <div>
                        <h2 className="text-2xl font-bold text-white">Городской Банк</h2>
                        <p className="text-gray-400">Управляйте вашими Городскими Кредитами (ГК)</p>
                    </div>
                    <div className="bg-dark-700/50 p-4 rounded-lg">
                        ...
                        <div className="text-center text-4xl text-transparent bg-clip-text bg-gradient-to-r from-green-300 via-emerald-400 to-teal-500 py-2 animate-text-glow">
                            <AnimatedBalance value={user.cityCredits} isCurrency={false} />
                        </div>
                    </div>
                </div>
            </Card>

...
import { MOCK_PROJECT_DATA, MOCK_GUILD_MEMBERS } from '../constants.ts';
import type { ProjectNode as ProjectNodeType, GuildMember } from '../types.ts';
import * as htmlToImage from 'html-to-image';
import { User, ShieldPlus, Link, Check, ChevronRight, Search, Maximize, Minimize, AlertCircle, Users as UsersIcon, Network, Download, X, Home, BarChart3, FlaskConical, BrainCircuit, HeartPulse, Lightbulb, HelpingHand, Target, XCircle, Building, Hotel, Coins } from 'lucide-react';
...
const SimulationModal: React.FC<{
    isOpen: boolean;
    onClose: () => void;
    onSelect: (member: GuildMember | { id: string, name: string, avatarUrl: string } | null) => void;
}> = ({ isOpen, onClose, onSelect }) => {
    const inactiveMembers = MOCK_GUILD_MEMBERS.filter(m => m.status === 'inactive');
...
const MemoizedProjectNode: React.FC<ProjectNodeProps> = React.memo((props) => {
    ...
    const typeColorClasses: Record<string, string> = { self: 'border-purple-500', spillover: 'border-cyan-400', clone: 'border-orange-400' };
    ...
    const healthColorClasses: Record<string, string> = { healthy: 'stroke-green-500', sleeping: 'stroke-yellow-500', problematic: 'stroke-red-500' };
                    ...
                    <div className="absolute top-full w-full h-10 pointer-events-none">
                       <svg viewBox={`0 0 ${isRoot ? 400 : 360} 40`} className="w-full h-full" preserveAspectRatio="none">
                          ...
                          {node.children.map((child, index) => {
                            ...
                            const healthClass = child.isFilled ? (healthColorClasses[child.branchHealth || ''] || 'stroke-gray-600') : 'stroke-gray-600';
...
const ProjectView: React.FC = () => {
    ...
    const { upgradeLevel } = currentProjectState;
    ...
    const matrixWithHealth = useMemo(() => {
        const calculateHealth = (node: ProjectNodeType): ProjectNodeType['branchHealth'] => {
            ...
            if (activityDates.length === 0) return 'problematic';
            ...
            if (daysSinceActivity <= 21) return 'sleeping';
            return 'problematic';
        };
        ...
        const traverse = (node: ProjectNodeType): ProjectNodeType => {
            ...
            newNode.branchHealth = calculateHealth(newNode);
            return newNode;
        };
    ...
    const handleUpgrade = (level: 2 | 3) => {
        setCurrentProjectState(prev => ({...prev, upgradeLevel: level}));
        const upgradeName = level === 2 ? "Дома" : "Отель";
        addToast(`Проект улучшен до уровня "${upgradeName}"!`, 'success');
    };
    ...
    const handleGetAiSuggestions = () => {
        ...
        if (aiSuggestions.length > 0) {
            setAiSuggestions([]);
             ...
             setTimeout(() => {
                setAiSuggestions([
                    { nodeId: "UCHILD6", reason: "Быстрый доход: Завершает второй уровень, принося максимальный доход.", type: "fast" },
                    { nodeId: "UCHILD2", reason: "Помощь гильдии: Усилит 'горячую' ветку Марии С., ускоряя ее прогресс.", type: "team" },
                    { nodeId: "UCHILD5", reason: "Стратегический ход: Открывает новую сильную ветку.", type: "strategic" },
                ]);
                addToast('AI-Советник готов! Оптимальные места подсвечены.', 'success');
            }, 1500);
        }
    };
    ...
    const handleSelectPlacement = (member: GuildMember | { id: string, name: string, avatarUrl: string } | null) => {
        ...
        const findAndReplace = (node: ProjectNodeType): boolean => {
             if (node.id === simulatingSlot.id) {
                Object.assign(node, {
                    ...
                    nodeType: 'self',
                    level: (member as GuildMember).level || 1,
                    investors: (member as GuildMember).investors || 0,
                    joinDate: new Date().toISOString(),
                    lastActivityDate: new Date().toISOString(),
                    children: [{ id: 'EMPTY_L', name: 'Свободно', avatarUrl: '', isFilled: false, upgradeLevel: 1, district: 'Жилой' }, { id: 'EMPTY_R', name: 'Свободно', avatarUrl: '', isFilled: false, upgradeLevel: 1, district: 'Жилой' }]
                });
                return true;
            }
            if (node.children) {
        ...
        setSimulationResult({
            ...
            effects: [
                `Вы получаете 500 ГК 'Арендной платы'.`,
                `Ваш партнер 'Мария С.' получает 'Городской контракт'.`,
                `Осталось 1 место до сдачи Проекта и получения бонуса 2000 ГК.`
            ]
        });
        setSimulatingSlot(null);
    };
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 p-4">
                ...
                <div className="xl:col-span-1 flex items-center gap-2 p-2 bg-dark-800 rounded-lg border border-dark-700 w-full justify-center">
                    <button onClick={() => { resetModes(); setHealthMapMode(p => !p); }} className={`px-3 py-1.5 text-xs rounded-md flex items-center transition-colors ${isHealthMapMode ? 'bg-brand-primary text-white' : 'hover:bg-dark-600'}`}>
                        <HeartPulse className="h-4 w-4 mr-1.5"/>Карта здоровья
                    </button>
                    <button onClick={handleGetAiSuggestions} className={`px-3 py-1.5 text-xs rounded-md flex items-center transition-colors ${aiSuggestions.length > 0 ? 'bg-brand-primary text-white' : 'hover:bg-dark-600'}`}>
                        <BrainCircuit className="h-4 w-4 mr-1.5"/>AI-Советник
                    </button>
                 ...
                 <div className="absolute top-4 left-4 z-10">
                    <Card className="!p-3">
                        ...
                        <div className="text-xs text-gray-400 mb-3">
                            <p>Статус: <span className={`font-semibold ${upgradeLevel === 1 ? 'text-gray-300' : upgradeLevel === 2 ? 'text-cyan-400' : 'text-yellow-400'}`}>
                                {upgradeLevel === 1 ? 'Участок' : upgradeLevel === 2 ? 'Дома' : 'Отель'}
                            </span></p>
                            <p>Бонус аренды: <span className="font-semibold text-green-400">{upgradeLevel > 1 ? '+25%' : 'Стандарт' }</span></p>
                            <p>Бонус за сдачу: <span className="font-semibold text-green-400">{upgradeLevel > 2 ? '+50%' : 'Стандарт' }</span></p>
                        </div>
                        <div className="space-y-2">
                            <Button onClick={() => handleUpgrade(2)} disabled={upgradeLevel >= 2 || filledSlots < 3} variant="secondary" className="w-full !text-xs !py-1.5">
                                <Building className="w-4 h-4 mr-1.5"/>Улучшить до "Домов"
                            </Button>
                            <Button onClick={() => handleUpgrade(3)} disabled={upgradeLevel >= 3 || filledSlots < 6} variant="secondary" className="w-full !text-xs !py-1.5">
                                <Hotel className="w-4 h-4 mr-1.5"/>Улучшить до "Отеля"
                            </Button>
                             <p className="text-[10px] text-gray-500 text-center pt-1">
                                {upgradeLevel === 1 && filledSlots < 3 ? `Нужно ${3 - filledSlots} инвестора для "Домов"` : ''}
                                {upgradeLevel === 2 && filledSlots < 6 ? `Нужно ${6 - filledSlots} инвестора для "Отеля"` : ''}
                            </p>
                        </div>
                    </Card>
                </div>
